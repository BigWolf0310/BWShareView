...
@interface BWShareView ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
{
    if (collectionView == self.collectionView) {
        BWItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:shareCell forIndexPath:indexPath];
        if (self.shareArray.count > 0) {
            [cell updateContent:self.shareArray[indexPath.item]];
        }
        return cell;
    } else {
        BWItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:itemCell forIndexPath:indexPath];
...
- (UIView *)bgView
{
    if (!_bgView) {
        ...
        _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, bgH + kTitleHeight)];
...
{
    if (!_shareTitleLabel) {
        _shareTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, kTitleHeight)];
...
{
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.shareTitleLabel.frame), [UIScreen mainScreen].bounds.size.width, 90) collectionViewLayout:self.layout];
        _collectionView.backgroundColor = RGBColor(245, 245, 245);
        _collectionView.delegate = self;
        _collectionView.dataSource = self;
        [_collectionView registerClass:[BWItemCollectionViewCell class] forCellWithReuseIdentifier:shareCell];
    }
...
- (UICollectionView *)collectionView2
{
    if (!_collectionView2) {
        ...
        _collectionView2 = [[UICollectionView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.collectionView.frame), [UIScreen mainScreen].bounds.size.width, height) collectionViewLayout:self.layout2];
...
- (UIButton *)cancleButton
{
    if (!_cancleButton) {
        ...
        _cancleButton.frame = CGRectMake(0, CGRectGetMaxY(self.collectionView2.frame), [UIScreen mainScreen].bounds.size.width, 50);
